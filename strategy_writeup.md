...
> *We have completely abandoned the Deep RL approach described below. PPO peaked at 342.0, while Mega Lucario v19 reached the verified rule-based peak of **958.8**. A 56-game v19 audit produced v24's Alakazam, Dragapult, Crustle, and Archaludon safeguards. A subsequent 32-game v24 ladder batch exposed two deterministic policy bugs—wrong-side offensive prize accounting and search-rollout attack-plan leakage—plus missing successor, printed-attack, forced-promotion, spread, and emergency-attachment evaluation. The current v25 candidate fixes those issues, gates spread on actual next-turn attack reachability, supports attach-then-retreat escapes, and evaluates up to three ranked opponent MAIN replies inside the existing time cap. Replay 90715746 further corrected defensive Hariyama gust chaining and preserves a safe Mega before hand-reset supporters remove it. It preserves the deck and remains unpromoted until it beats 958.8 on the real ladder. The rest of this document details the legacy RL architecture for historical reference.*
