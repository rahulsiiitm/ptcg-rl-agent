...
The current candidate is **v25**. It retains v24's Alakazam, Dragapult, Crustle, and Archaludon gains, then fixes the replay-proven failures from v24's 32-game ladder batch: offensive prize counts now use our remaining prizes, search rollouts cannot overwrite the real attack plan, bench-target wins require gusting before the attack is rewarded, and terminal defense evaluates printed attacks, reachable spread attacks, visible successor attackers, forced-promotion candidates, Grimmsnarl spread, and Tera-discounted Azumarill. Planned lethal and attach-to-safe-retreat Energy placements outrank generic Riolu development. Replay 90715746 added defensive Hariyama gusting that cannot immediately undo itself, plus safe Mega evolution before Carmine or Lillie would discard/shuffle the only visible evolution. The bounded search evaluates up to three ranked opponent MAIN replies while forced prompts still obey the engine's `maxCount`. The deck remains unchanged so the real ladder can isolate policy quality. v25 is not considered better than v19 until it beats 958.8. See [`eval/v19_replay_audit.md`](eval/v19_replay_audit.md) for the earlier forensic baseline.
